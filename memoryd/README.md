...
- Claude Code + Codex + OpenClaw 三端共享同一 scope（Plan 1-2.5）
- Session capture 自动后台 + DURA 4 准则 LLM 提升候选（Plan 3）
- 长期记忆 6 类型 + decay / soft-forget + digest 周复盘（Plan 3）
- 敏感作用域 AES-256-GCM + OS keyring + append-only 审计（Plan 4）
- 多电脑同步：用户自配同步盘镜像裸 .md（Plan 6）
- 本机轻量 Web Dashboard（浏览-only）+ textual digest TUI（Plan 7）
...
## Import + memory-searcher (Plan 8)

memoryd v0.8.0 加 4 个单向 import 子命令 + memory-searcher sub-agent 模板。spec §4.7 #28 + §4.2 #5。**v1 完成**。

### 一次性 import 旧记忆

```bash
# 从 CC 的 CLAUDE.md 按段切（heuristic H2/H3 → fact/playbook/warning/decision/preference）
memoryd import claude-md ~/.claude/CLAUDE.md

# 从 ~/.claude/projects/<proj>/memory/ 整段导（跳过 MEMORY.md index）
memoryd import auto-memory ~/.claude/projects/-Users-you/memory/

# 从 Codex AGENTS.md（与 claude-md 同 heuristic）
memoryd import agents-md ~/.codex/AGENTS.md

# 从 mcp-memory-service 的 memories.json（兼容 wrapper dict 或裸数组）
...
- `--dry-run` — 不写，仅出 plan JSON
- `--force` — 覆盖同 slug
- `--source-tag=<custom>` — 覆盖 frontmatter source

**全部单向**——memoryd 不双向同步 CLAUDE.md / AGENTS.md / auto-memory（spec §6）。导入后这些原文件继续由 CC / Codex 各自管。

### memory-searcher sub-agent

memoryd 提供一个 CC sub-agent 模板：用 haiku-4-5 + Read+Grep 工具搜 `~/.local/share/memoryd/scopes/`，输出 ≤ 500 token 的 JSON 命中结果。主 context 零占用。

```bash
memoryd setup install-memory-searcher           # → ~/.claude/agents/memory-searcher.md
memoryd setup install-memory-searcher --force   # 覆盖
memoryd setup install-memory-searcher --target=./.claude/agents/  # 项目级
```

装好后用户在 CC 里说"找一下上次关于 X 的"等 prompt，CC 主智能体可自动调用 memory-searcher 检索；sensitive scope 自动跳过（.md.enc 不读）。

### Limitations

- import 切分是 heuristic（不依赖 LLM）；v2 视用户反馈加 `--llm` opt-in
...
Plan 1-8 全部 merged。memoryd v0.8.0 用户契约 spec §4 列的 32 项功能全部交付：
- §4.1 自动后台 capture（三端 SessionEnd / FS-watch / OpenClaw SDK）
- §4.2 智能体自然交互（search_memory MCP + memory-searcher sub-agent）
- §4.3 主动控制（CLI memory list / search / show / promote / merge / digest / audit）
- §4.4 周期复盘（digest + decay + TTL + soft-forget）
- §4.5 敏感保护（mark-sensitive / grant / audit append-only + prev_hash chain）
- §4.6 跨工具协调（CC + Codex + OpenClaw 共享同一 scope，原生记忆不接管）
- §4.7 数据所有权（Markdown source of truth + SQLite index + 多电脑 sync + import）
- §4.8 浏览界面（CLI + TUI + 轻量本地 Web Dashboard + Basic Memory schema 对齐）

MCP 工具数 8/12，预算未用完，留 v2 扩展余地。
