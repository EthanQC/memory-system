...
!!! tip "更快的方式"
    把 [让 AI 帮你装](../user/install-via-ai.md) 那段 prompt 复制给你的 AI，它会替你跑完所有命令。

!!! note "不需要 LLM API key"
    本页所有步骤完全本地工作，不需要配 LLM key。LLM 只有 DURA 评分、知识图谱抽取、weekly identity 等可选功能才用得到。
